...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	if (mi_cfg->fod_hbm_enabled) {
		pr_info("fod_hbm_enabled set, skip\n");
		goto exit;
	}
...
int dsi_panel_enable(struct dsi_panel *panel)
{
	...
	if ((panel->bl_config.xiaomi_f4_41_flag && panel->dc_enable && panel->dc_demura_threshold) || (panel->bl_config.xiaomi_f4_36_flag && panel->dc_enable)) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DISP_DC_ON);
		if (rc)
			pr_err("[%s] failed to send DSI_CMD_SET_DISP_DC_ON cmd, rc=%d\n",
					panel->name, rc);
