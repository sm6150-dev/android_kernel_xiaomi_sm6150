...
static void panelon_dimming_enable_delayed_work(struct work_struct *work)
{
	...
	if (display) {
		...
		dsi_panel_set_disp_param(panel, DISPPARAM_DIMMING);
...
static int dsi_panel_set_disp_param(struct dsi_panel *panel, int param)
{
	int rc = 0;
	uint32_t temp = 0;
	u32 fod_backlight = 0;
...
int panel_disp_param_send(struct dsi_display *display, int param_type)
{
  ...
  rc = dsi_panel_set_disp_param(panel, param_type);
  return rc;
}
