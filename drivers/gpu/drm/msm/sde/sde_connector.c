...
int sde_connector_update_hbm(struct sde_connector *c_conn)
{
	...
	if (!hbm_overlay) {
		if (dsi_display->panel->fod_dimlayer_hbm_enabled) {
			...
			if (dsi_display->panel->fod_dimlayer_hbm_enabled) {
				if (dsi_display->panel->doze_enabled)
					dsi_panel_set_fod_hbm(
						dsi_display->panel, true);
			} else {
				dsi_panel_set_fod_hbm(dsi_display->panel,
						      false);
			}
		}
		...
		if (!dsi_display->panel->fod_dimlayer_hbm_enabled) {
			...
			dsi_panel_set_fod_hbm(dsi_display->panel, true);
...
void sde_connector_fod_notify(struct drm_connector *conn)
{
	...
	if (dsi_display->panel->hbm_ntfy_skip_flag) {
		...
		pr_info("%s: icon notify need skip hbm_ntfy_skip_flag[%d]",
			__func__, dsi_display->panel->hbm_ntfy_skip_flag);
		return;
	}
